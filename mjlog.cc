...
- (bool)computSeat:(char[4])result {
  ...
  if ([NSSet setWithArray:sorted].count != 4)
    return NO;
  for (int i = 0; i < 4; i++)
...
- (void)startHand:(enum MjOya)oya
  ...
  [currentHand addObjectsFromArray:@[ h[0][12], h[1][12], h[2][12], h[3][12] ]];
}
...
BOOL isFetchTileAction(const std::string &string, int *number) {
  if (string[0] != 'T' && string[0] != 'U' && string[0] != 'V' &&
      string[0] != 'W')
    return NO;
  auto sc = [NSScanner scannerWithString:@(string.substr(1).c_str())];
  [sc scanInt:number];
  return [sc isAtEnd];
}
